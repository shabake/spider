...
def create_agent(api_key=None, base_url="https://api.deepseek.com/v1", db_path=None, strategy_mode=False, confirm_enabled=True, profile: dict = None, max_turns=30, plan_mode=False, team_mode=False):
    """创建预配置的 Agent 实例
...
async def run_task(task: str, api_key=None, base_url=None, db_path=None, strategy_mode=False, confirm_enabled=True, profile=None, profile_name=None, max_turns=30, plan_mode=False, team_mode=False):
    """执行单次任务"""
    try:
        agent = create_agent(api_key, base_url, db_path, strategy_mode=strategy_mode, confirm_enabled=confirm_enabled, profile=profile, max_turns=max_turns, plan_mode=plan_mode, team_mode=team_mode)
        await agent.run(task, cli=cli)
    except AuthError as e:
        print(f"\n{e}")


async def interactive_mode(api_key=None, base_url=None, db_path=None, strategy_mode=False, confirm_enabled=True, profile=None, profile_name=None, max_turns=30, plan_mode=False, team_mode=False):
...
def main():
    ...
    parser.add_argument("--team", action="store_true",
                        help="团队协作模式（可协调 coding/reviewer 等角色子 Agent）")
    ...
    if args.team:
        cli.muted("👥 团队协作模式（可调度 coding/reviewer 等角色）")
    if args.no_confirm:
